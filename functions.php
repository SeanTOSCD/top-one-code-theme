...
function toc_setup() {
	...
	load_theme_textdomain( 'toc', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary', 'toc' ),
			'footer-menu' => __( 'Footer', 'toc' ),
		)
	);
...
add_action( 'after_setup_theme', 'toc_setup' );

// Set the content width in pixels
function toc_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'toc_content_width', 856 );
}
add_action( 'after_setup_theme', 'toc_content_width', 0 );

// Register sidebar area.
function toc_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'toc' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'toc' ),
...
add_action( 'widgets_init', 'toc_widgets_init' );

// Enqueue scripts and styles.
function toc_scripts() {
	wp_enqueue_style( 'toc-style', get_stylesheet_uri(), array(), THEME_VERSION );
	wp_enqueue_script( 'toc-navigation', get_template_directory_uri() . '/assets/js/scripts.min.js', array(), THEME_VERSION, true );
 ...
 * @return void
 */
function toc_disable_classic_theme_styles() {
	wp_deregister_style( 'classic-theme-styles' );
	wp_dequeue_style( 'classic-theme-styles' );
}
add_filter( 'wp_enqueue_scripts', 'toc_disable_classic_theme_styles', 100 );
